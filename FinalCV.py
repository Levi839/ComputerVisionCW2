...
# Panorama stiching application by Levi, Saman and Bob
class Stitcher:
    def __init__(self):
    ...
    def compute_descriptors(self, img):
        """
        Input: standard Image matrix
        Uses the sift algorithm to run through the process of extracting the keypoints and descriptors e.g. greyscale blur then keypoint extraction etc
        Output: Keypoints and descriptors(features of the image)
    ...
    def matching(self,keypoints_l, keypoints_r, descriptors_l, descriptors_r,match_threshold):
        ...
        '''
        matches = [] # Store corresponding matches in a matrix for later use/ output
        for k1, desc_l in enumerate(descriptors_l): # K1 is keypoint 1 from the left image, desc_l is the descriptor from the left image
            distances = [] # matrix storing the Euiclidian distances between points a and b
            for k2, desc_r in enumerate(descriptors_r): #K2 is keypoint 2 from the right image, desc_r is the descriptor from the right image
                eucli_distance = np.linalg.norm(desc_l - desc_r) # Calculate the euclidian distance between point a and b
                distances.append((eucli_distance, k2)) # Store the calculated distance and its coresponding point within the matrix

            distances.sort(key=lambda x: x[0]) # Sort the distances in order


            # Ratio test of the matches, checks that matches fall within the threshold aswell as closest matches between two points
            # Summary: It just filters out the bad matches
    ...
    def draw_matches(self, img_left, keypoints_l, img_right, keypoints_r, matches):
        '''
            Input :Both images and there keypoints and matches
            Connects correspondences between images with lines and draws said lines
            output: no output(Void method)
        '''
        # Draw matches
